...
const SysReq = ({ platform, title, systemReqData }) => {
  return (
    <div className="grid grid-rows-[auto_1fr] gap-2 bg-gray-200 rounded-md p-2">
      ...
      <ul className="grid grid-cols-3 gap-1 h-full">
        {systemReqData.map((reqData, index) =>
          reqData.length === 2 ? (
            <li
              key={index}
              className={`flex flex-col border-[1px] border-gray-900/10 px-1 rounded-sm ${index === 0 && !reqData[0].toLowerCase().includes('os') ? 'col-span-3 border-none' : ''}`}
            >
